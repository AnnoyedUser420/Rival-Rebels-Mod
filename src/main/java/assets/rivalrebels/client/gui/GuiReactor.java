...
public class GuiReactor extends GuiContainer {
    ...
    private final TileEntityReactor ter;
    private final Block[] machineslist = {RivalRebels.forcefieldnode, RivalRebels.reactive};
    private final Block[] machines = {null, null, null, null};
    private final boolean[] onmachines = new boolean[machines.length];
    private final boolean[] enabledmachines = new boolean[machineslist.length];
    private final boolean[] prevenabledmachines = new boolean[machineslist.length];
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        if (ter.on && ter.fuel != null && ter.core != null) {
            float radius = 10;
            if (ter.fuel.getTagCompound() != null)
                radius += (((((ItemRod) ter.fuel.getItem()).power * ((ItemCore) ter.core.getItem()).timemult) - ter.fuel.getTagCompound().getInteger("fuelLeft")) / (((ItemRod) ter.fuel.getItem()).power * ((ItemCore) ter.core.getItem()).timemult)) * 30;
            melttick = 30;
            float brightness = 0;
            ...
            if (ter.fuel.getItem() == RivalRebels.nuclearelement)
                drawNoiseSphere(0.9f, 1f, 0.1f, 0f, 1f, 0.1f, frame, 4, (int) radius, (int) (50 - radius), resolution, 0.02f, brightness);
            if (ter.fuel.getItem() == RivalRebels.hydrod)
                drawNoiseSphere(1f, 1f, 1f, 0.7f, 0f, 1f, frame, 4, (int) radius, (int) (50 - radius), resolution, 0.02f, brightness);
            if (ter.fuel.getItem() == RivalRebels.redrod)
                drawNoiseSphere(1f, 0.8f, 0f, 1f, 0f, 0f, frame, 4, (int) radius, (int) (50 - radius), resolution, 0.02f, brightness);
