...
public class EntityRhodes extends Entity {
    ...
    private static final HashSet blocklist = new HashSet();
    ...
    private final Queue<RhodesAction> actionqueue = new LinkedList<RhodesAction>();
    ...
    public void onUpdate() {
        ...
        if (health > 0) {
            ...
            if (rider != null) {
                ...
                if (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT && rider == Minecraft.getMinecraft().thePlayer)
                    PacketDispatcher.packetsys.sendToServer(new RhodesJumpPacket(this, RivalRebels.proxy.spacebar(), RivalRebels.proxy.a(), RivalRebels.proxy.w(), RivalRebels.proxy.d(), RivalRebels.proxy.c(), RivalRebels.proxy.f(), RivalRebels.proxy.s(), RivalRebels.proxy.x(), RivalRebels.proxy.z(), RivalRebels.proxy.g()));
            }
            if (passenger1 != null) {
                float offset = 1.62f;
                if (worldObj.isRemote) {
            ...
            if (!worldObj.isRemote) {
                if (health == 0)
                    PacketDispatcher.packetsys.sendToAll(new TextPacket("RivalRebels.Status " + getName() + " RivalRebels.meltdown" + (rider == null ? "" : " " + rider.getCommandSenderName())));
                if (ticksExisted % 5 == 0) PacketDispatcher.packetsys.sendToAll(new RhodesPacket(this));
                if (health < -100) {
                    setDead();
                }
        ...
        switch (ac) {
            ...
            case 10:
                if (teamToRaid == 1 ?
                        ...
                        : (teamToRaid == 2 && (RivalRebels.round.sigmaHealth > 0 && worldObj.getBlock(RivalRebels.round.sObjx, RivalRebels.round.sObjy, RivalRebels.round.sObjz) == RivalRebels.sigmaobj))) {
                    rightthighpitch = approach(rightthighpitch, 0);
                    leftthighpitch = approach(leftthighpitch, 0);
                    rightshinpitch = approach(rightshinpitch, 0);
                    leftshinpitch = approach(leftshinpitch, 0);
    ...
    private void shootRocketsAtBestTarget(float syaw, float cyaw) {
        ...
        if (te != null && !nuke) {
            ...
            if (Math.abs(leftarmyaw - yaw) >= 0.001f) {
                leftarmyaw += Math.max(-3, Math.min(3, (yaw - leftarmyaw)));
                if (Math.abs(leftarmyaw - yaw) < 3f) {
                }
                else pointing = false;
            }
            if (Math.abs(leftarmpitch - pitch) >= 0.001f) {
                leftarmpitch += Math.max(-3, Math.min(3, (pitch - leftarmpitch)));
                if (Math.abs(leftarmpitch - pitch) < 3f) {
                }
                else pointing = false;
            }
    ...
    private float getPriority(Entity e) {
        ...
        if ((e instanceof EntityRhodes && (RivalRebels.rhodesFF && (RivalRebels.rhodesCC || ((EntityRhodes) e).colorType != colorType))) || e instanceof EntityB2Spirit)
            return 800;
        if (e.boundingBox.getAverageEdgeLength() > 3)
            return (float) (e.boundingBox.getAverageEdgeLength() * 3 + 500 + e.height);
        return 0;
    }
