...
public class RivalRebelsRound implements IMessage {
    ...
    private final int objRadius = 29;
    private final int objRadius2 = 29 * 29;
    ...
    public void updateInvisible() {
        ...
        for (int i = 0; i < playerlist.size(); i++) {
            ...
            if (e.inventory.armorInventory[3] != null && e.inventory.armorInventory[3].getItem() == RivalRebels.camera)
                setInvisible(e);
        }
        for (int i = players.size() - 1; i >= 0; i--) {
            PlayerInvisibility t = players.get(i);
    ...
    public void load() {
        ...
        if (server != null) {
            ...
            } //just in case teams already exist etc
        ...
        for (int x1 = -spawnRadius; x1 < spawnRadius; x1++) {
            ...
            for (int z1 = -spawnRadius; z1 < spawnRadius; z1++) {
                ...
                for (int y1 = -5; y1 < hrs; y1++) {
                    int YY = y1 * y1 + ZZ;
                    if ((YY > rs4 && YY < spawnRadius2) || ((y1 == -2 || y1 == -3 || y1 == -4) && YY < rs1))
                        world.setBlock(cSpawnx + x1, 200 + y1, cSpawnz + z1, RivalRebels.fshield);
                }
            }
        }
