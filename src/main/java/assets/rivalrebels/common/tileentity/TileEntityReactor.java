...
public class TileEntityReactor extends TileEntity implements IInventory {
    ...
    public void updateEntity() {
        ...
        if (side == Side.CLIENT) {
            ...
            if (on && core != null && fuel != null && core.getItem() instanceof ItemCore && fuel.getItem() instanceof ItemRod) {
                if (!prevOn && on)
                    RivalRebelsSoundPlayer.playSound(worldObj, 21, 3, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5);
                else {
                    tick++;
                    if (on && tick % 39 == 0)
                        RivalRebelsSoundPlayer.playSound(worldObj, 21, 2, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, 0.9f, 0.77f);
                }
    ...
    public boolean isItemValidForSlot(int i, ItemStack itemstack) {
        if (itemstack == null || !(itemstack.getItem() instanceof ItemRod) || !(itemstack.getItem() instanceof ItemCore))
            return false;
        if (i == 0) {
            return fuel == null || !on;
        }
