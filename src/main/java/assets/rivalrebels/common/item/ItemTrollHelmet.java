...
public class ItemTrollHelmet extends ItemArmor {
    ...
    public boolean onItemUse(ItemStack item, EntityPlayer player, World world, int x, int y, int z, int s, float px, float py, float pz) {
        ...
        if (item.stackSize == 0) {
            ...
            world.playSoundEffect((float) x + 0.5F, (float) y + 0.5F, (float) z + 0.5F, RivalRebels.flag2.stepSound.func_150496_b(), (RivalRebels.flag2.stepSound.getVolume() + 1.0F) / 2.0F, RivalRebels.flag2.stepSound.getPitch() * 0.8F);
            --item.stackSize;
            return true;
        } else {
            return false;
